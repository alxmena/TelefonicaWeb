<%@ Page Title="" Language="C#" MasterPageFile="~/Main.master" AutoEventWireup="true" CodeBehind="ManageUsers.aspx.cs" Inherits="ATCPortal.Master.ManageUsers" %>
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
    <dx:ASPxGridView ID="grdUsers" ClientInstanceName="grdUsers" runat="server" AutoGenerateColumns="False" DataSourceID="sqlMain" KeyFieldName="UserId" Theme="Mulberry" OnRowInserting="grdUsers_RowInserting" OnRowValidating="grdUsers_RowValidating" OnRowDeleting="grdUsers_RowDeleting" OnRowUpdating="grdUsers_RowUpdating" OnCellEditorInitialize="grdUsers_CellEditorInitialize" Width="100%" >
        <Columns>
            ...
            <dx:GridViewDataCheckColumn FieldName="IsApproved" VisibleIndex="4" ShowInCustomizationForm="True">
                <EditItemTemplate>
                    <dx:ASPxCheckBox ID="chkIsApproved" runat="server" OnLoad="chkIsApproved_Load">
                    </dx:ASPxCheckBox>
                </EditItemTemplate>
            ...
            <dx:GridViewDataComboBoxColumn FieldName="CompanyID" Caption="Company" VisibleIndex="11">
                <PropertiesComboBox DataSourceID="sqlCompany" TextField="Name" ValueField="ID">
                    ...
                    <ClientSideEvents SelectedIndexChanged="function(s, e) {grdUsers.GetEditor('BranchID').PerformCallback(s.GetValue());}" />
                </PropertiesComboBox>
            </dx:GridViewDataComboBoxColumn>
            <dx:GridViewDataComboBoxColumn FieldName="BranchID" Caption="Branch" VisibleIndex="12">
                <PropertiesComboBox DataSourceID="sqlAllBranches" TextField="Name" ValueField="ID">
                    <ValidationSettings RequiredField-IsRequired="true" Display="Dynamic" />
                </PropertiesComboBox>
            </dx:GridViewDataComboBoxColumn>
            <dx:GridViewDataCheckColumn FieldName="IsManager" VisibleIndex="13" ShowInCustomizationForm="True">
            </dx:GridViewDataCheckColumn>
        </Columns>
        <Settings ShowFilterRow="True" ShowFilterRowMenu="True" ShowFilterRowMenuLikeItem="True" />
    </dx:ASPxGridView>
    <asp:SqlDataSource ID="sqlMain" runat="server" ConnectionString="<%$ ConnectionStrings:ApplicationServices %>"
        SelectCommand="SELECT aspnet_Users.UserId, aspnet_Users.UserName, aspnet_Membership.Email, aspnet_Membership.IsApproved, aspnet_Membership.IsLockedOut, aspnet_Membership.LastLoginDate, aspnet_Membership.LastLockoutDate, aspnet_Membership.FailedPasswordAttemptCount, aspnet_Membership.FailedPasswordAttemptWindowStart, aspnet_Users.CompanyID, aspnet_Users.BranchID,aspnet_Users.IsManager
        FROM aspnet_Users INNER JOIN aspnet_Membership ON aspnet_Users.UserId = aspnet_Membership.UserId"></asp:SqlDataSource>
    ...
    <asp:SqlDataSource ID="sqlAllBranches" runat="server" ConnectionString="<%$ ConnectionStrings:DBConnection %>" SelectCommand="SELECT [ID], [Name] FROM [tblBranch]"></asp:SqlDataSource>
